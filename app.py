...
def logout():
    ...
    flash(f"Goodbye, {user.username} you have been logged out", "success")
...
def user_likes(user_id):
    """Show and list the posts liked by a user"""
    ...
    messages = (Message
                .query
                .filter(Message.id.in_(likes_list))
                .order_by(Message.timestamp.desc())
                .limit(100)
                .all())
...
def unlike_message(message_id):
    ...
    msg = Message.query.get(message_id)
    g.user.likes.pop(g.user.likes.index(msg))
    db.session.commit()
...
def homepage():
    ...
    if g.user:
        #get a list of user class objects for the users the current 
        #user is following
        following = g.user.following

        self_and_following_ids = []
        self_and_following_ids.append(g.user.id)
        for user in following:
            self_and_following_ids.append(user.id)
